...
```{r DailyTotalSteps, echo=TRUE,fig.path = "figures/" }
...
```{r AveDailyActivity, echo=TRUE,fig.path = "figures/"}
...
The 5 mins interval containing the maximum number of steps `r intdat[which.max(intdat$steps),] `  
...
Total missing values in the dataset: `r colSums(is.na(alldata)) `
...
```{r DailyTotStepwFill, echo=TRUE,fig.path = "figures/"}
...
```{r Compareplots, echo=TRUE,fig.path = "figures/"}
 h1col <- rgb(1,0,0,0.2)
 h2col <- rgb(0,0,1,0.2)
...
```{r WeekendWeekday, echo=TRUE,fig.path = "figures/"}
library(lattice)
